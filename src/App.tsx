...
// ── Design tokens ─────────────────────────────────────────────────────────────
const C={pageBg:"#f0f0f0",headerBg:"#2c2c2c",cardBg:"#ffffff",cardBorder:"#d0d0d0",rowEven:"#f8f8f8",rowOdd:"#ffffff",label:"#555",mutedText:"#888",matrixCellBg:"#3a3a3a",matrixHeaderBg:"#2c2c2c"};
...
function Btn({onClick,children,variant="primary",small,full,disabled}){
  const V={primary:{background:"#2c2c2c",color:"#fff"},success:{background:"#2e7d32",color:"#fff"},danger:{background:"#c62828",color:"#fff"},ghost:{background:"transparent",color:"#555",border:"1px solid #ccc"},accent:{background:"#455a64",color:"#fff"},info:{background:"#1565c0",color:"#fff"}};
...
function NewCatModal({cats,onSave,onClose}){
  ...
  function validate(){
    ...
    if(!/^[A-Z]{2,6}$/.test(p)){setErr("Il codice deve essere 2-6 lettere maiuscole.");return;}
    if(cats[p]){setErr(`Il codice "${p}" esiste già → "${cats[p]}". Sceglierne un altro.`);return;}
    if(Object.values(cats).some(v=>v.toLowerCase()===l.toLowerCase())){setErr(`La categoria "${l}" esiste già.`);return;}
    setErr("");onSave(p,l);
  }
  return(<ModalBox title="➕  Nuova categoria" onClose={onClose} width={440}>
    <p style={{fontSize:12,color:"#666",marginBottom:16,lineHeight:1.5}}>Il codice determina il prefisso (es. <strong>MKT</strong> → MKT-001, MKT-002…). Usa 2-6 lettere maiuscole.</p>
    <Field label="Codice categoria * (es. MKT)"><input style={IS} value={prefix} onChange={e=>setPrefix(e.target.value.toUpperCase())} placeholder="es. MKT" maxLength={6} autoFocus/></Field>
    ...
    {prefix.trim()&&label.trim()&&!err&&<div style={{background:"#e8f5e9",border:"1px solid #a5d6a7",borderRadius:8,padding:"9px 13px",fontSize:12,color:"#1b5e20",marginBottom:12}}>✔ Categoria <strong>{label.trim()}</strong> ({prefix.trim().toUpperCase()}) — primo codice: <strong>{prefix.trim().toUpperCase()}-001</strong></div>}
...
// ── Portale Modal (edit/new) ──────────────────────────────────────────────────
function PortaleModal({portale,onSave,onClose}){
  const empty={nome:"",url:"",username:"",password:"",note:""};
  const [form,setForm]=useState(portale||empty);
  const [showPwd,setShowPwd]=useState(false);
  const upd=k=>e=>setForm(p=>({...p,[k]:e.target.value}));
  const wc=wordCount(form.note);
  const isNew=!portale;
  return(<ModalBox title={isNew?"➕  Nuovo portale":"✏️  Modifica portale"} onClose={onClose} width={520}>
    ...
    <Field label={`Note (max 150 parole — ${wc}/150)`} hint="Campo alfanumerico libero">
      <textarea style={{...TA,borderColor:wc>150?"#e53935":"#bbb"}} value={form.note} onChange={e=>{const words=e.target.value.trim().split(/\s+/).filter(Boolean);if(words.length<=150||e.target.value.length<form.note.length)upd("note")(e);}} placeholder="Note sul portale, contatti, istruzioni di accesso..."/>
...
export default function App(){
  ...
  const [portali,setPortali]= useState(INIT_PORTALI);
  ...
  const [portaleModal,setPortaleModal]= useState(null); // null | "new" | {portale obj}
  ...
  function addCat(prefix,label){setCats(p=>({...p,[prefix]:label}));setShowNewCat(false);toast_(`Categoria "${label}" (${prefix}) creata.`);}
  ...
  // ── Portali anagrafica ────────────────────────────────────────────────────
  function savePortale(form){
    setPortali(p=>{
      const idx=p.findIndex(x=>x.nome===form.nome);
      if(idx>=0){const n=[...p];n[idx]=form;return n;}
      return [...p,form];
    });
    // se nome non è ancora in clients, aggiungilo
    if(!clients.includes(form.nome))setClients(p=>[...p,form.nome]);
    setPortaleModal(null);
    toast_(`Portale "${form.nome}" salvato.`);
  }
  function deletePortale(nome){
    setPortali(p=>p.filter(x=>x.nome!==nome));
    setConfirmDel(null);
    toast_(`Portale "${nome}" rimosso dall'anagrafica.`,"warn");
  }
  ...
  return(<div style={{minHeight:"100vh",background:C.pageBg,fontFamily:"'IBM Plex Sans',sans-serif",color:"#222"}}>
    <style>{`
      ...
      *{box-sizing:border-box;}
      ...
      .pwd-cell{font-family:'IBM Plex Mono',monospace;font-size:11px;cursor:pointer;user-select:none;}
    `}</style>
    ...
    {confirmDel&&<ConfirmDialog msg={confirmDel.type==="doc"?`Eliminare "${docs.find(d=>d.id===confirmDel.payload)?.nome}"?\nTutti i flag associati verranno rimossi.`:confirmDel.type==="portale"?`Rimuovere "${confirmDel.payload}" dall'anagrafica?`:`Eliminare il portale "${confirmDel.payload}"?\nTutti i flag associati verranno rimossi.`} onConfirm={()=>{if(confirmDel.type==="doc")deleteDoc(confirmDel.payload);else if(confirmDel.type==="portale")deletePortale(confirmDel.payload);else deleteClient(confirmDel.payload);}} onCancel={()=>setConfirmDel(null)}/>}
    ...
    <div style={{maxWidth:1600,margin:"0 auto",padding:"20px 24px"}}>
      ...
      {tab==="anagrafica"&&(<div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <input className="flt" placeholder="🔍 Cerca portale o URL..." value={filterPortale} onChange={e=>setFilterPortale(e.target.value)} style={{width:220}}/>
        ...
        <p style={{fontSize:11,color:"#aaa",marginBottom:10,fontStyle:"italic"}}>🔒 La password è mascherata — clicca 👁 per visualizzarla. Clicca su URL per aprire il portale.</p>
        <div style={{overflowX:"auto",borderRadius:11,border:"1px solid #ddd",background:"#fff",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:"#f5f5f5"}}>
              {["Cliente","URL Portale","Username","Password","Note","Azioni"].map(h=>(
                <th key={h} style={{padding:"10px 13px",textAlign:h==="Azioni"?"center":"left",fontWeight:700,fontSize:10,letterSpacing:"0.07em",textTransform:"uppercase",color:"#666",borderBottom:"2px solid #e0e0e0",whiteSpace:"nowrap"}}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {filteredPortali.map((p,i)=>(
                <PortaleRow key={p.nome+i} portale={p} rowBg={i%2===0?C.rowEven:C.rowOdd} TD={TD}
                  onEdit={()=>setPortaleModal(p)}
                  onDelete={()=>setConfirmDel({type:"portale",payload:p.nome})}/>
              ))}
            </tbody>
          </table>
        </div>
      ...
      {tab==="maschera"&&(<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(310px,1fr))",gap:18}}>
          <MascheraCard num="1" title="Nuovo documento" subtitle="Aggiunge riga allo Scadenzario · opz. assegna portale" color="#37474f" btnLabel="Inserisci documento" onSubmit={()=>{
            ...
            <Field label="Assegna a portale (opz.)"><select style={IS} value={fDoc.cliente} onChange={e=>setFDoc(f=>({...f,cliente:e.target.value}))}>
              <option value="">— nessuno —</option>
              {clients.map(c=><option key={c} value={c}>{c}</option>)}
            </select></Field>
          </MascheraCard>

          <MascheraCard num="2" title="Nuovo portale / cliente" subtitle="Aggiunge colonna alla Matrice + scheda Anagrafica" color="#4a5568" btnLabel="Inserisci portale" onSubmit={()=>{
            if(!fCli.nome.trim()){toast_("Inserire il nome del portale.","err");return;}
            insertClient(fCli.nome.trim());
            setPortaleModal({nome:fCli.nome.trim(),url:"",username:"",password:"",note:""});
            setFCli({nome:""});
          }}>
            ...
            <p style={{fontSize:11,color:"#888",marginTop:8,lineHeight:1.5}}>Dopo aver inserito il nome si aprirà automaticamente la scheda per aggiungere URL, credenziali e note.</p>
          </MascheraCard>
...
// ── PortaleRow — gestisce show/hide password ──────────────────────────────────
function PortaleRow({portale,rowBg,TD,onEdit,onDelete}){
  ...
  return(<tr className="rh" style={{background:rowBg}}>
    ...
    <td style={{...TD,maxWidth:280}}>
      {portale.url?(<a href={portale.url} target="_blank" rel="noopener noreferrer" style={{color:"#1565c0",textDecoration:"none",fontSize:11,fontFamily:"'IBM Plex Mono',monospace",wordBreak:"break-all"}}>{portale.url}</a>):<span style={{color:"#ccc"}}>—</span>}
    </td>
    ...
    <td style={{...TD,whiteSpace:"nowrap"}}>
      {portale.password?(<div style={{display:"flex",alignItems:"center",gap:6}}>
        <span className="pwd-cell" style={{color:show?"#222":"#bbb",letterSpacing:show?"0":"3px"}}>{show?portale.password:"••••••••"}</span>
        <button onClick={()=>setShow(s=>!s)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:"#888",padding:"0 2px"}} title={show?"Nascondi":"Mostra"}>{show?"🙈":"👁"}</button>
    ...
    <td style={{...TD,textAlign:"center",whiteSpace:"nowrap"}}>
      <button className="ab ed" title="Modifica" onClick={onEdit}>✏️</button>{" "}
      <button className="ab dl" title="Rimuovi dall'anagrafica" onClick={onDelete}>🗑️</button>
    </td>
  </tr>);
}
