...
const sb = {
  ...
  async deleteAll(table) {
    ...
    if (!r.ok) {
      // fallback: delete by known column
      await fetch(`${SUPABASE_URL}/rest/v1/${table}?nome=neq.___IMPOSSIBLE___`, {
        method: "DELETE",
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
      });
    }
...
function nextDocCode(docs,prefix){
  const used=docs.filter(d=>d.id.startsWith(prefix+"-")&&/^\d+$/.test(d.id.split("-")[1])).map(d=>parseInt(d.id.split("-")[1]));
...
// ── Excel export ──────────────────────────────────────────────────────────────
function exportExcel(docs,clients,flags,portali){
  ...
  const s3=[["Cliente","URL Portale","Username","Password","Note"]];
  portali.forEach(p=>s3.push([p.nome,p.url,p.username,p.password,p.note]));
  const ws3=XLSX.utils.aoa_to_sheet(s3);ws3["!cols"]=[25,45,30,20,40].map(w=>({wch:w}));
  XLSX.utils.book_append_sheet(wb,ws3,"Anagrafica Portali");
...
function EditClientModal({name,docs,flags,onSave,onClose}){
  ...
  return(<ModalBox title="✏️  Modifica portale" onClose={onClose} width={420}>
    <div style={{background:"#f5f5f5",border:"1px solid #ddd",borderRadius:9,padding:"9px 14px",marginBottom:16,fontSize:12,color:"#555"}}>Documenti associati: <strong>{cnt}</strong></div>
    <Field label="Nome portale *"><input style={IS} value={nome} onChange={e=>setNome(e.target.value)} autoFocus/></Field>
...
function NewCatModal({cats,onSave,onClose}){
  ...
  function validate(){
    const p=prefix.trim().toUpperCase(),l=label.trim();
    if(!p){setErr("Inserire il codice.");return;}
    if(!l){setErr("Inserire il nome.");return;}
    if(!/^[A-Z]{2,6}$/.test(p)){setErr("Codice: 2-6 lettere maiuscole.");return;}
    if(cats[p]){setErr(`Codice "${p}" già esistente → "${cats[p]}".`);return;}
    if(Object.values(cats).some(v=>v.toLowerCase()===l.toLowerCase())){setErr(`Categoria "${l}" già esistente.`);return;}
    setErr("");onSave(p,l);
  }
  return(<ModalBox title="➕  Nuova categoria" onClose={onClose} width={440}>
    <Field label="Codice categoria *"><input style={IS} value={prefix} onChange={e=>setPrefix(e.target.value.toUpperCase())} placeholder="es. MKT" maxLength={6} autoFocus/></Field>
    <Field label="Nome categoria *"><input style={IS} value={label} onChange={e=>setLabel(e.target.value)} placeholder="es. Marketing"/></Field>
    ...
    <details style={{marginBottom:16}}><summary style={{fontSize:11,color:"#888",cursor:"pointer"}}>Categorie esistenti ({Object.keys(cats).length})</summary>
      <div style={{marginTop:8,display:"flex",flexWrap:"wrap",gap:6}}>{Object.entries(cats).sort((a,b)=>a[0].localeCompare(b[0])).map(([k,v])=>(<span key={k} style={{background:"#f0f0f0",border:"1px solid #ddd",borderRadius:5,padding:"2px 8px",fontSize:11,fontFamily:"'IBM Plex Mono',monospace"}}><strong>{k}</strong> → {v}</span>))}</div>
    </details>
...
function PortaleModal({portale,onSave,onClose}){
  const [form,setForm]=useState(portale||{nome:"",url:"",username:"",password:"",note:""});
  const [showPwd,setShowPwd]=useState(false);
  const upd=k=>e=>setForm(p=>({...p,[k]:e.target.value}));
  const wc=wordCount(form.note);
  return(<ModalBox title={portale?"✏️  Modifica portale":"➕  Nuovo portale"} onClose={onClose} width={520}>
    <Field label="Nome cliente / portale *"><input style={IS} value={form.nome} onChange={upd("nome")} autoFocus placeholder="es. ENEL"/></Field>
    ...
    <Field label="Password">
      <div style={{position:"relative"}}>
        <input type={showPwd?"text":"password"} style={{...IS,paddingRight:40}} value={form.password} onChange={upd("password")} placeholder="••••••••"/>
        <button onClick={()=>setShowPwd(s=>!s)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#888",fontSize:16}}>{showPwd?"🙈":"👁"}</button>
      </div>
    </Field>
    <Field label={`Note (max 150 parole — ${wc}/150)`}>
      <textarea style={{...TA,borderColor:wc>150?"#e53935":"#bbb"}} value={form.note} onChange={e=>{const w=e.target.value.trim().split(/\s+/).filter(Boolean);if(w.length<=150||e.target.value.length<form.note.length)upd("note")(e);}} placeholder="Note sul portale..."/>
      {wc>150&&<div style={{fontSize:10,color:"#e53935",marginTop:3}}>Limite 150 parole superato.</div>}
    </Field>
...
function PortaleRow({portale,rowBg,TD,onEdit,onDelete}){
  ...
  return(<tr className="rh" style={{background:rowBg}}>
    ...
    <td style={{...TD,whiteSpace:"nowrap"}}>{portale.password?(<div style={{display:"flex",alignItems:"center",gap:6}}>
      <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:show?"#222":"#bbb",letterSpacing:show?"0":"3px"}}>{show?portale.password:"••••••••"}</span>
      <button onClick={()=>setShow(s=>!s)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:"#888",padding:"0 2px"}}>{show?"🙈":"👁"}</button>
    ...
    <td style={{...TD,textAlign:"center",whiteSpace:"nowrap"}}>
      <button className="ab ed" onClick={onEdit}>✏️</button>{" "}
      <button className="ab dl" onClick={onDelete}>🗑️</button>
    </td>
  </tr>);
}
...
function LoadingScreen({msg}){
  return(<div style={{minHeight:"100vh",background:"#f0f0f0",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
    <div style={{fontFamily:"'DM Serif Display',serif",fontSize:24,color:"#2c2c2c"}}>Monitoraggio Scadenze</div>
    <div style={{width:40,height:40,border:"4px solid #ddd",borderTop:"4px solid #2c2c2c",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}></div>
...
function SetupScreen({onSetup}){
  ...
  async function test(){
    ...
    try{
      const r=await fetch(`${url}/rest/v1/docs?select=id&limit=1`,{headers:{apikey:key,Authorization:`Bearer ${key}`}});
      if(r.status===200||r.status===406){onSetup(url.replace(/\/$/,""),key);}
      else setErr(`Errore ${r.status}. Verifica URL e chiave.`);
    }catch(e){setErr("Connessione fallita. Verifica l'URL.");}
    setTesting(false);
  }
  return(<div style={{minHeight:"100vh",background:"#f0f0f0",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
    <div style={{background:"#fff",borderRadius:16,padding:36,width:520,maxWidth:"100%",boxShadow:"0 8px 32px rgba(0,0,0,0.1)"}}>
      <div style={{fontFamily:"'DM Serif Display',serif",fontSize:22,color:"#222",marginBottom:8}}>Configura Supabase</div>
      <p style={{fontSize:13,color:"#666",marginBottom:24,lineHeight:1.6}}>Inserisci le credenziali del tuo progetto Supabase per abilitare il salvataggio persistente dei dati.</p>
      <div style={{marginBottom:16}}>
        <label style={{display:"block",fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"#555",marginBottom:5}}>Project URL</label>
        <input style={{width:"100%",background:"#fff",border:"1px solid #bbb",borderRadius:7,padding:"9px 13px",color:"#222",fontSize:13,outline:"none",fontFamily:"'IBM Plex Mono',monospace",boxSizing:"border-box"}} value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://xxxxxxxxxxxx.supabase.co"/>
      </div>
      <div style={{marginBottom:20}}>
        <label style={{display:"block",fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"#555",marginBottom:5}}>Anon Key</label>
        <input style={{width:"100%",background:"#fff",border:"1px solid #bbb",borderRadius:7,padding:"9px 13px",color:"#222",fontSize:13,outline:"none",fontFamily:"'IBM Plex Mono',monospace",boxSizing:"border-box"}} value={key} onChange={e=>setKey(e.target.value)} placeholder="eyJhbGciOiJIUzI1NiIs..."/>
      </div>
      {err&&<div style={{background:"#ffebee",border:"1px solid #ef9a9a",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#c62828",marginBottom:16}}>{err}</div>}
      <button onClick={test} disabled={!url||!key||testing} style={{width:"100%",background:"#2c2c2c",color:"#fff",border:"none",borderRadius:8,padding:"12px",fontWeight:800,fontSize:13,cursor:(!url||!key||testing)?"not-allowed":"pointer",opacity:(!url||!key||testing)?0.5:1,fontFamily:"'IBM Plex Mono',monospace"}}>
        {testing?"Connessione in corso...":"▶  Connetti e avvia"}
      </button>
      <p style={{fontSize:11,color:"#aaa",marginTop:16,lineHeight:1.6,textAlign:"center"}}>Le credenziali vengono salvate solo nel browser locale (localStorage).</p>
    </div>
  </div>);
}
...
export default function App(){
  const [sbConfig,setSbConfig]=useState(()=>{
    try{const c=localStorage.getItem("sb_config");return c?JSON.parse(c):null;}catch{return null;}
  });
  ...
  const [portaleModal,setPortaleModal]=useState(null);
  ...
  const [filterPortale,setFilterPortale]=useState("");

  function toast_(msg,type="ok"){setToast({msg,type});setTimeout(()=>setToast(null),3600);}

  // ── Override sb with config ──────────────────────────────────────────────
  function getSb(cfg){
    const u=cfg.url,k=cfg.key;
    return{
      async get(table){
        const r=await fetch(`${u}/rest/v1/${table}?select=*`,{headers:{apikey:k,Authorization:`Bearer ${k}`}});
        if(!r.ok)throw new Error(`${r.status}`);
        return r.json();
      },
      async upsert(table,data){
        const r=await fetch(`${u}/rest/v1/${table}`,{method:"POST",headers:{apikey:k,Authorization:`Bearer ${k}`,"Content-Type":"application/json",Prefer:"resolution=merge-duplicates"},body:JSON.stringify(data)});
        if(!r.ok){const t=await r.text();throw new Error(t);}
      },
      async delete(table,col,val){
        const r=await fetch(`${u}/rest/v1/${table}?${col}=eq.${encodeURIComponent(val)}`,{method:"DELETE",headers:{apikey:k,Authorization:`Bearer ${k}`}});
        if(!r.ok)throw new Error(`${r.status}`);
      }
    };
  }
  ...
  const loadData=useCallback(async(cfg)=>{
    ...
    try{
      ...
      if(dbDocs.length===0){
        ...
        setDocs(sortDocs(d.map(x=>({id:x.id,cat:x.cat,nome:x.nome,durata:x.durata,dataEmissione:x.data_emissione||""}))));
        setClients(c.map(x=>x.nome));
        const fm={};f.forEach(x=>{fm[flagKey(x.doc_id,x.client_nome)]=true;});setFlags(fm);
        setPortali(p.map(x=>({nome:x.nome,url:x.url||"",username:x.username||"",password:x.password||"",note:x.note||""})));
        if(ct.length>0){const cm={};ct.forEach(x=>{cm[x.prefix]=x.label;});setCats(cm);}
      } else {
        setDocs(sortDocs(dbDocs.map(x=>({id:x.id,cat:x.cat,nome:x.nome,durata:x.durata,dataEmissione:x.data_emissione||""}))));
        setClients(dbClients.map(x=>x.nome));
        const fm={};dbFlags.forEach(x=>{fm[flagKey(x.doc_id,x.client_nome)]=true;});setFlags(fm);
        setPortali(dbPortali.map(x=>({nome:x.nome,url:x.url||"",username:x.username||"",password:x.password||"",note:x.note||""})));
        if(dbCats.length>0){const cm={};dbCats.forEach(x=>{cm[x.prefix]=x.label;});setCats(cm);}
      }
      setAppState("ready");
  ...
  async function seedData(api){
    ...
    if(flagRows.length>0){
      ...
      for(let i=0;i<flagRows.length;i+=500)await api.upsert("flags",flagRows.slice(i,i+500));
    }
    ...
    await api.upsert("cats",Object.entries(INIT_CATS).map(([prefix,label])=>({prefix,label})));
  }
  ...
  useEffect(()=>{
    if(sbConfig)loadData(sbConfig);
    else setAppState("setup");
  },[]);

  // ── Helpers ────────────────────────────────────────────────────────────────
  function api(){return getSb(sbConfig);}
  async function withSave(fn,successMsg){
    setSaving(true);
    try{await fn();if(successMsg)toast_(successMsg);}
    catch(e){toast_(`Errore: ${e.message}`,"err");}
    finally{setSaving(false);}
  }

  // ── Docs CRUD ──────────────────────────────────────────────────────────────
  async function insertDoc(d,cli){
    ...
    await withSave(async()=>{
      await api().upsert("docs",[{id,cat:nd.cat,nome:nd.nome,durata:nd.durata,data_emissione:nd.dataEmissione||null}]);
      if(cli)await api().upsert("flags",[{doc_id:id,client_nome:cli}]);
      setDocs(prev=>sortDocs([...prev,nd]));
      if(cli)setFlags(f=>({...f,[flagKey(id,cli)]:true}));
    },`"${nd.nome}" inserito (${id})${cli?" → "+cli:""}.`);
  }
  async function saveDoc(u){
    await withSave(async()=>{
      await api().upsert("docs",[{id:u.id,cat:u.cat,nome:u.nome,durata:u.durata,data_emissione:u.dataEmissione||null}]);
      setDocs(p=>sortDocs(p.map(d=>d.id===u.id?u:d)));
      setEditDoc(null);
    },`"${u.nome}" aggiornato.`);
  }
  async function deleteDoc(id){
    ...
    await withSave(async()=>{
      await api().delete("docs","id",id);
      await api().delete("flags","doc_id",id);
      setDocs(p=>p.filter(x=>x.id!==id));
      setFlags(p=>{const n={...p};Object.keys(n).forEach(k=>{if(k.startsWith(id+"___"))delete n[k];});return n;});
      setConfirmDel(null);
    },`"${d?.nome}" eliminato.`);
  }
  async function addCat(prefix,label){
    await withSave(async()=>{
      await api().upsert("cats",[{prefix,label}]);
      setCats(p=>({...p,[prefix]:label}));
      setShowNewCat(false);
    },`Categoria "${label}" (${prefix}) creata.`);
  }

  // ── Clients CRUD ───────────────────────────────────────────────────────────
  async function insertClient(nome){
    if(clients.includes(nome)){toast_("Portale già presente.","err");return;}
    await withSave(async()=>{
      await api().upsert("clients",[{nome}]);
      setClients(p=>[...p,nome]);
    },`"${nome}" aggiunto.`);
  }
  async function saveClient(old,nw){
    if(nw!==old&&clients.includes(nw)){toast_("Nome già esistente.","err");return;}
    await withSave(async()=>{
      if(old!==nw){
        // rename: insert new, update flags, delete old
        await api().upsert("clients",[{nome:nw}]);
        // update flags in batch: delete old, re-insert with new name
        const toMove=Object.keys(flags).filter(k=>k.endsWith("___"+old));
        if(toMove.length){
          await api().delete("flags","client_nome",old);
          await api().upsert("flags",toMove.map(k=>({doc_id:k.split("___")[0],client_nome:nw})));
        }
        await api().delete("clients","nome",old);
        setClients(p=>p.map(c=>c===old?nw:c));
        setFlags(p=>{const n={};Object.entries(p).forEach(([k,v])=>{n[k.endsWith("___"+old)?k.replace("___"+old,"___"+nw):k]=v;});return n;});
        setPortali(p=>p.map(x=>x.nome===old?{...x,nome:nw}:x));
      }
      setEditClient(null);
    },`Rinominato in "${nw}".`);
  }
  async function deleteClient(name){
    await withSave(async()=>{
      await api().delete("clients","nome",name);
      await api().delete("flags","client_nome",name);
      setClients(p=>p.filter(c=>c!==name));
      setFlags(p=>{const n={...p};Object.keys(n).forEach(k=>{if(k.endsWith("___"+name))delete n[k];});return n;});
      setConfirmDel(null);
    },`"${name}" eliminato.`);
  }
  async function assignDoc(docNome,cli){
    const doc=docs.find(d=>d.nome===docNome);
    if(!doc||!cli){toast_("Seleziona portale e documento.","err");return;}
    await withSave(async()=>{
      await api().upsert("flags",[{doc_id:doc.id,client_nome:cli}]);
      setFlags(f=>({...f,[flagKey(doc.id,cli)]:true}));
    },`"${docNome}" → "${cli}".`);
  }
  async function toggleFlag(docId,cli){
    const k=flagKey(docId,cli);
    const has=!!flags[k];
    await withSave(async()=>{
      if(has)await api().delete("flags","doc_id",docId+"&client_nome=eq."+encodeURIComponent(cli));
      else await api().upsert("flags",[{doc_id:docId,client_nome:cli}]);
      setFlags(f=>({...f,[k]:!has}));
    });
  }

  // ── Portali CRUD ───────────────────────────────────────────────────────────
  async function savePortale(form){
    await withSave(async()=>{
      await api().upsert("portali",[{nome:form.nome,url:form.url||null,username:form.username||null,password:form.password||null,note:form.note||null}]);
      setPortali(p=>{const idx=p.findIndex(x=>x.nome===form.nome);if(idx>=0){const n=[...p];n[idx]=form;return n;}return[...p,form];});
      if(!clients.includes(form.nome)){await api().upsert("clients",[{nome:form.nome}]);setClients(p=>[...p,form.nome]);}
      setPortaleModal(null);
    },`Portale "${form.nome}" salvato.`);
  }
  async function deletePortale(nome){
    await withSave(async()=>{
      await api().delete("portali","nome",nome);
      setPortali(p=>p.filter(x=>x.nome!==nome));
      setConfirmDel(null);
    },`"${nome}" rimosso dall'anagrafica.`);
  }
  ...
  const filteredPortali=useMemo(()=>portali.filter(p=>!filterPortale||p.nome.toLowerCase().includes(filterPortale.toLowerCase())||p.url.toLowerCase().includes(filterPortale.toLowerCase())),[portali,filterPortale]);
  const suggestedCode=useMemo(()=>{if(!fDoc.cat)return"";const prefix=Object.entries(cats).find(([k,v])=>v===fDoc.cat)?.[0];if(!prefix)return"";return nextDocCode(docs,prefix);},[fDoc.cat,docs,cats]);
  const STATI=["VALIDO","IN SCADENZA","ATTENZIONE","URGENTE","SCADUTO"];
  const TABS=[{id:"scadenzario",label:"📄  Scadenzario"},{id:"clienti",label:"👥  Matrice Portali"},{id:"anagrafica",label:"🔐  Anagrafica Portali"},{id:"maschera",label:"✏️  Inserimento"}];
  const TD={padding:"9px 13px",borderBottom:"1px solid #ececec",verticalAlign:"middle"};

  if(appState==="setup")return(<SetupScreen onSetup={handleSetup}/>);
  if(appState==="loading")return(<LoadingScreen msg="Caricamento dati dal database..."/>);
  if(appState==="error")return(<div style={{minHeight:"100vh",background:"#f0f0f0",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
    <div style={{fontSize:32}}>❌</div>
    <div style={{fontSize:16,color:"#c62828",fontWeight:700}}>Errore di connessione al database</div>
    <Btn onClick={()=>{localStorage.removeItem("sb_config");setSbConfig(null);setAppState("setup");}} variant="danger">Riconfigura Supabase</Btn>
  </div>);
  ...
  return(<div style={{minHeight:"100vh",background:C.pageBg,fontFamily:"'IBM Plex Sans',sans-serif",color:"#222"}}>
    <style>{`
      ...
      *{box-sizing:border-box;}
      ...
      input:focus,select:focus,textarea:focus{border-color:#555!important;box-shadow:0 0 0 2px rgba(0,0,0,0.08);}
      ...
      .ab:hover{border-color:#ccc;} .ab.ed:hover{background:#e3f2fd;color:#1565c0;} .ab.dl:hover{background:#ffebee;color:#c62828;}
    ...
    <div style={{background:C.headerBg,padding:"0 24px"}}>
      <div style={{maxWidth:1600,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:58}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {saving&&<span style={{fontSize:11,color:"#aaa",fontFamily:"'IBM Plex Mono',monospace"}}>💾 Salvataggio...</span>}
          ...
          <Btn onClick={()=>{localStorage.removeItem("sb_config");window.location.reload();}} variant="ghost" small>⚙</Btn>
        </div>
      </div>
    ...
    {portaleModal&&<PortaleModal portale={portaleModal==="new"?null:portaleModal} onSave={savePortale} onClose={()=>setPortaleModal(null)}/>}
    {confirmDel&&<ConfirmDialog msg={confirmDel.type==="doc"?`Eliminare "${docs.find(d=>d.id===confirmDel.payload)?.nome}"?\nTutti i flag verranno rimossi.`:confirmDel.type==="portale"?`Rimuovere "${confirmDel.payload}" dall'anagrafica?`:`Eliminare "${confirmDel.payload}"?\nTutti i flag verranno rimossi.`} onConfirm={()=>{if(confirmDel.type==="doc")deleteDoc(confirmDel.payload);else if(confirmDel.type==="portale")deletePortale(confirmDel.payload);else deleteClient(confirmDel.payload);}} onCancel={()=>setConfirmDel(null)}/>}
    ...
    <div style={{maxWidth:1600,margin:"0 auto",padding:"20px 24px"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            ...
            <tbody>{filteredDocs.map((d,i)=>{
              ...
              return(<tr key={d.id+i} className="rh" style={{background:rowBg}}>
                ...
                <td style={{...TD,textAlign:"center",whiteSpace:"nowrap"}}>
                  <button className="ab ed" onClick={()=>setEditDoc(d)}>✏️</button>{" "}
                  <button className="ab dl" onClick={()=>setConfirmDel({type:"doc",payload:d.id})}>🗑️</button>
                </td>
              </tr>);
            })}</tbody>
          </table>
      ...
      {tab==="anagrafica"&&(<div>
        ...
        <p style={{fontSize:11,color:"#aaa",marginBottom:10,fontStyle:"italic"}}>🔒 Password mascherata — clicca 👁 per visualizzarla. Clicca URL per aprire il portale.</p>
        <div style={{overflowX:"auto",borderRadius:11,border:"1px solid #ddd",background:"#fff",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
      ...
      {tab==="maschera"&&(<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(310px,1fr))",gap:18}}>
          <MascheraCard num="1" title="Nuovo documento" subtitle="Aggiunge riga allo Scadenzario" color="#37474f" btnLabel="Inserisci documento" onSubmit={()=>{if(!fDoc.nome.trim()||!fDoc.cat){toast_("Inserire categoria e nome.","err");return;}insertDoc({id:fDoc.id,cat:fDoc.cat,nome:fDoc.nome,dataEmissione:fDoc.dataEmissione,durata:fDoc.durata},fDoc.cliente||null);setFDoc(f=>({...f,id:"",nome:"",dataEmissione:"",durata:"",cliente:""}));}}>
            <Field label="Categoria *"><select style={IS} value={fDoc.cat} onChange={e=>setFDoc(f=>({...f,cat:e.target.value,id:""}))}>
              <option value="">— seleziona —</option>{Object.entries(cats).sort((a,b)=>a[1].localeCompare(b[1])).map(([k,v])=><option key={k} value={v}>{v} ({k})</option>)}
            </select></Field>
            <Field label="Codice" hint={fDoc.cat?`Lascia vuoto → ${suggestedCode}`:"Seleziona prima la categoria"}><input style={IS} value={fDoc.id} onChange={e=>setFDoc(f=>({...f,id:e.target.value}))} placeholder={suggestedCode||"automatico"}/></Field>
            ...
            <Field label="Durata (mesi) — 0 = nessuna scadenza"><input type="number" style={IS} value={fDoc.durata} onChange={e=>setFDoc(f=>({...f,durata:e.target.value}))} placeholder="6" min={0}/></Field>
